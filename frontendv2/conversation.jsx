...
const Conversation = ({ setPage }) => {
  ...
  React.useEffect(() => {
    // simulate streaming for the second message
    const full = "AI_ANSWER";
  ...
  return (
    <div className="convo">
      ...
      {messages.map(m => m.role === "user"
        ...
        : <AIMsg key={m.id} m={m} streaming={streaming} setPage={setPage}/>)}
    ...
    {m.followup ? (
      ...
      <div className="msg__body">
        ...
        <div className="trace">
          ...
          <span className="trace__line"/>
          <span style={{cursor: "pointer", color: "var(--accent)"}} onClick={() => setPage("graph")}>view in graph ↗</span>
        </div>
      </div>
    )}
  </div>
