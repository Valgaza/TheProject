...
const NODE_TYPES: Record<string, { label: string, color: string, fill: string }> = {
...
const GRAPH = (() => {
  const nodes: any[] = [];
  const edges: any[] = [];
  const add = (id: string, label: string, type: string, x: number, y: number, r?: number) => nodes.push({ id, label, type, x, y, r: r || 6 });
  const link = (s: string, t: string, label: string) => edges.push({ s, t, label });
...
export default function Graph() {
  ...
  const [pinned, setPinned] = useState<string | null>("q3");
